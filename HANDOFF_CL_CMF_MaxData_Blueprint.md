...
| **API CMF Bancos v3** | ✅ macros via `chile_macros_loader.py` (mindicador + optional `CMF_API_KEY`) | TMC needs CMF key |
| **BCCh Series bancarias** | ✅ `chile_bcch_loader.py` (CSV zip; Playwright vs Imperva) | Product cuts `CL_BCCH_*` |
| **Ratings** | ✅ `chile_ratings_loader.py` → `data/cl_bank_ratings.json` | Feller still curated (login wall) |
